...
function Executor() {
  ...
  this.debug = Debug('strong-central:driver:bluemix:' + this._id);
...
function _onRequest(msg, callback) {
  ...
  switch (msg.cmd) {
    case 'register-container':
      return this._onRegisterContainer(msg, callback);
  }
...
/**
 * Handels 'register-container' notification from the strong-heroku-runner
 * module. The notification is issued when a new container starts on Heroku
 * and includes details about the HerokuResource and container size.
 */
function _onRegisterContainer(msg, callback) {
  ...
  BMServiceBinding.findById(msg.bmServiceBindingId, function(err, binding) {
    if (err || !binding) return callback({error: 'Invalid token'});
    async.parallel([
      binding.executor.bind(binding),
      binding.serverService.bind(binding)
    ], function(err, res) {
      if (err) return callback({error: 'Invalid token'});
      var executor = res[0];
      var serverService = res[1];
      var instance = {
        ...
        setSize: -1,
        cpus: msg.cpus,
        applicationName: binding.app_guid,
        npmModules: {},
        agentVersion: msg.agentVersion,
        version: msg.version,
        herokuResourceId: msg.herokuResourceId,
      };
      Instance.create(instance, function(err, inst) {
        if (err) return callback({error: 'Unable to register instance'});
        var supervisorUrl = url.parse(meshApp.get('BM:supervisorUrl'));
...
Executor.prototype._onRegisterContainer = _onRegisterContainer;
...
/**
 * When a container disconnects and fails to re-connect in time, it is
 * considered terminated and the instance model is updated with the stopTime.
 */
function _onInstanceExit(instanceId) {
  ...
  Instance.findById(instanceId, function(err, inst) {
    if (err) {
      return self.debug(
        fmt('Container exited but instance not found %s', err.message)
      );
    }
    ...
    inst.updateAttributes({
      ...
      if (err) {
        self.debug(
          fmt('Container exited but error updating model: %s', err.message)
        );
      }
    });
  });
